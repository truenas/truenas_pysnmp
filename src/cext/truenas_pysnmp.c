...
/* Free v3 fields we allocated on a stack session. NULL-safe.
 * securityName is a borrowed Python pointer, not ours to free. */
static void
		     ...
		     trap_error_t *err)
{
	...
	/* Borrowed pointer from Python — snmp_add() strdup's it internally */
	...
	/* Engine ID setup — caller must resolve fallback before calling */
	eid = engine_id;
...
/* Send trap and handle result. Caller must validate first. */
static PyObject *
